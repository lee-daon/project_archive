...
async function calculatePrices(variants, priceConfig, productId) {
    try {
        ...
        const {
            ...
            freeShipping,               // 무료배송 여부 (getConfig에서 freeShipping으로 전달됨)
            includeImportDuty           // 관부가세 포함 여부
        } = priceConfig;

        // 쿠팡 판매 수수료 (일반적으로 카테고리별로 다르지만 평균 8-15%)
        const coupangSellingFee = 12; // 12% (쿠팡 기본 수수료 - 카테고리별로 다를 수 있음)
        ...
        const calculatedVariants = variants.map(variant => {
            ...
            // 3. 총 비용 계산 (무료배송인 경우 배송비 포함)
            let totalCostKRW = costBeforeDutyKRW + dutyKRW + vatKRW;
            
            if (freeShipping) { // 무료배송이면 배송비를 상품 가격에 포함
                totalCostKRW += parseFloat(deliveryFee);
            }

            // 4. 목표 판매가 계산 (판매처 수수료 및 마진 고려)
            const profitMarginRate = profitMargin / 100; // 마진율
            const sellingFeeRate = coupangSellingFee / 100; // 쿠팡 판매수수료율
        ...
        // 배송비 정보 (독립적으로 반환)
        const deliveryInfo = {
            deliveryFee: freeShipping ? 0 : parseFloat(deliveryFee), // freeShipping 사용
            freeShipping: freeShipping // 필드명 통일
        };
        ...
        return {
            ...
            deliveryInfo: {
                deliveryFee: priceConfig.freeShipping ? 0 : (parseFloat(priceConfig.deliveryFee) || 0),
                freeShipping: priceConfig.freeShipping || false
            }
        };
    }
}
...
function generateRandomDiscount(priceConfig, productId) {
    try {
        ...
        console.log(`[${productId}] 생성된 할인율: ${discountRate}%`);
        return discountRate;
