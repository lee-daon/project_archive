...
/**
 * 11번가 등록을 위한 초기 JSON 데이터를 생성하는 함수
 * @param {Object} jsonData - JSON 형태의 상품 데이터
 * @param {number} elevenstoreCatId - 11번가 카테고리 ID
 * @param {Object} priceConfig - 가격 설정 데이터
 * @param {Object} elevenstoreConfig - 11번가 설정 데이터
 ...
 */
export async function createInitialJson(jsonData, elevenstoreCatId, priceConfig, elevenstoreConfig, detailPageConfig, userid, productid, productGroupCode) {
    try {
        // A-Z 옵션 적용
        ...
        // 1. 옵션 이미지와 이름 매핑 생성 (처리된 optionSchema 구조 사용)
        ...
        // 1.1. 옵션 이미지 URL 정리 (generateDetailContent에 전달하기 전)
        ...
        return {
            success: true,
            message: '11번가 createInitialJson 함수 모든 단계 완료 - 최종 JSON 생성됨',
            initialJson: initialJson
        };

    } catch (error) {
        console.error('11번가 createInitialJson 함수에서 오류 발생:', error);
        throw error;
    }
}
