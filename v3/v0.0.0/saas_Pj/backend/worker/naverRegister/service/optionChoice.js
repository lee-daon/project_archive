/**
 * 네이버 등록용 옵션 선택 및 가격 필터링 모듈
 * priceSettingLogic에 따라 다른 전략으로 옵션을 필터링합니다.
 */

/**
 * 가격 설정 로직에 따라 옵션을 필터링하고 대표 가격을 결정합니다.
 * @param {object} processedJsonData - InitialJson에서 처리된 JSON 데이터
 * @param {string} priceSettingLogic - 가격 설정 로직 ('low_price', 'many', 'ai')
 * @param {number} discountRate - 할인율 (0-99)
 * @returns {Promise<{ representativePrice: number, filteredJsonData: object }>} 대표 가격 및 필터링된 데이터
 */
export async function optionChoice(processedJsonData, priceSettingLogic, discountRate) {
  // 입력 유효성 검사
  ...
  switch (priceSettingLogic) {
    ...
    case 'ai':
      return await aiStrategy(filteredProcessedJsonData, discountRate);
    default:
      throw new Error(`지원하지 않는 가격 설정 로직입니다: ${priceSettingLogic}`);
  }
}

/**
 * low_price 전략: 가장 낮은 가격 기준으로 +50%까지만 포함
...
async function lowPriceStrategy(processedJsonData, discountRate) {
  ...
  // 3. 대표 가격은 가장 낮은 가격의 할인 전 가격
  const representativePrice = lowestVariant.originalPrice;
  
  // 4. 네이버 정책에 따른 허용 범위 계산 (할인 후 가격 기준 ±50%)
  const maxAllowedDiscountedPrice = lowestDiscountedPrice * 1.5; // +50%
  const minAllowedDiscountedPrice = lowestDiscountedPrice * 0.5; // -50% (실제로는 최저가이므로 의미없음)
  ...
  // 7. 각 variant에 priceGap 추가
  const finalVariants = filteredVariants.map(variant => {
    const { originalPrice, ...variantInfo } = variant;
    let priceGap = Math.round((variant.calculatedPrice - representativeDiscountedPrice) / 10) * 10;
    if (Math.abs(priceGap) <= 100) {
      priceGap = 0;
    }
    return { ...variantInfo, priceGap };
  });
...
/**
 * many 전략: 가장 많은 옵션이 포함되도록 하는 대표가격 설정
 * @param {object} processedJsonData - 처리된 JSON 데이터
 * @param {number} discountRate - 할인율
 * @returns {Promise<{ representativePrice: number, filteredJsonData: object }>}
 */
async function manyStrategy(processedJsonData, discountRate) {
  ...
  for (const anchorVariant of variantsWithPrices) {
    ...
    // 네이버 정책에 따른 허용 범위 계산 (할인 후 가격 기준 ±50%)
    const maxAllowedDiscountedPrice = anchorDiscountedPrice * 1.5;
    const minAllowedDiscountedPrice = anchorDiscountedPrice * 0.5;
  ...
  if (!bestAnchorVariant) {
    console.warn('many 전략: 최적 기준 옵션을 찾을 수 없습니다. 원본 데이터를 반환합니다.');
    return {
      representativePrice: 0,
      filteredJsonData: processedJsonData
    };
  ...
  // 각 variant에 priceGap 추가 (10원 단위 반올림)
...
/**
 * ai 전략: 기존 priceFilter.js 로직 그대로 사용
 * @param {object} processedJsonData - 처리된 JSON 데이터
 * @param {number} discountRate - 할인율
 * @returns {Promise<{ representativePrice: number, filteredJsonData: object }>}
 */
async function aiStrategy(processedJsonData, discountRate) {
  // 1. 각 variant의 할인 전 가격 계산
  const variantsWithPrices = processedJsonData.variants.map((variant) => {
    const originalPrice = discountRate === 0
      ? variant.calculatedPrice
  ...
  // 2. 최적의 "기준 옵션" 찾기 (priceFilter.js 로직)
  let bestAnchorVariant = null;
  let maxIncludedCount = -1;
  let finalFilteredVariantsInfo = [];
  ...
  for (const anchorVariant of variantsWithPrices) {
    ...
    const delta = anchorOriginalPrice * 0.5; // 기준 옵션의 할인 전 가격의 50%
  ...
  if (!bestAnchorVariant) {
    console.warn('ai 전략: 최적 기준 옵션을 찾을 수 없습니다. 원본 데이터를 반환합니다.');
    return {
      representativePrice: 0,
      filteredJsonData: processedJsonData
    };
