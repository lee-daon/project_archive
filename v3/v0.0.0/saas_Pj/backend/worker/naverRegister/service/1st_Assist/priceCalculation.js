...
async function calculatePrices(variants, priceConfig, productId) {
    try {
        ...
        const {
            ...
            includeDeliveryFee,         // 배송비 포함 여부
            includeImportDuty           // 관부가세 포함 여부
        } = priceConfig;

        // 네이버 판매 수수료 (고정값 또는 설정에서 가져오기)
        const naverSellingFee = 6; // 6% (네이버 기본 수수료)
        ...
        const calculatedVariants = variants.map(variant => {
            ...
            // 가격 정보가 유효하지 않은 경우 기본값 사용
            if (isNaN(itemPriceCNY) || itemPriceCNY <= 0) {
                console.warn(`[가격 계산] 상품 ID ${productId}의 옵션 가격이 유효하지 않습니다. 기본값 사용:`, variant);
                // 기본 가격 설정 (예: 100 CNY)
                itemPriceCNY = 100;
            }
            ...
            const sellingFeeRate = naverSellingFee / 100; // 판매수수료율
        ...
        // 배송비 정보 (독립적으로 반환)
        const deliveryInfo = {
            deliveryFee: includeDeliveryFee ? 0 : parseFloat(deliveryFee),
        ...
        return {
            ...
            deliveryInfo: {
                ...
                includeDeliveryFee: priceConfig.includeDeliveryFee || false
            }
        };
    }
}
