 ...
 */
export const checkSellerBan = async (userid, sellerId) => {
  try {
    const [results] = await promisePool.execute(
      'SELECT ban, updated_at FROM ban_seller WHERE userid = ? AND sellerid = ?',
      [userid, sellerId]
    );
 ...
 */
export const checkShopBan = async (userid, shopId) => {
  try {
    const [results] = await promisePool.execute(
      'SELECT ban, updated_at FROM ban_shop WHERE userid = ? AND shopid = ?',
      [userid, shopId]
    );
...
export const checkBanStatus = async (userid, sellerId, shopId) => {
  try {
    const sellerBanInfo = await checkSellerBan(userid, sellerId);
    const shopBanInfo = await checkShopBan(userid, shopId);
