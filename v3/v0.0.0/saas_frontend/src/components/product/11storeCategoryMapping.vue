...
<script>
import elevenStCategories from '@/assets/11st_categories_transformed.json'
import elevenStOverseasCategories from '@/assets/11st_overseas_categories_flat.json'

// 11번가 카테고리 캐시 (첫 로드 이후 재사용)
let elevenStFlatCache = null
let elevenStOverseasFlatCache = null

export default {
  name: 'ElevenStoreCategoryMappingModal',
  props: {
    isVisible: {
      type: Boolean,
      default: false
    ...
    platform: {
      type: String,
      default: '11st',
      validator: value => ['naver', 'coopang', '11st'].includes(value)
    },
    productName: {
      type: String,
      default: ''
  ...
  data() {
    return {
      activeTab: 'normal', // 'normal' | 'global'
      searchTerm: '',
  ...
  methods: {
    ...
    loadCategories(callback) {
      ...
      const cache = isNormalTab ? elevenStFlatCache : elevenStOverseasFlatCache
      const categories = isNormalTab ? elevenStCategories : elevenStOverseasCategories
      
      if (cache) {
        this.allCategories = cache
      ...
      setTimeout(() => {
        this.allCategories = categories // 이미 평면 구조
        
        // 캐시에 저장
        if (isNormalTab) {
          elevenStFlatCache = this.allCategories
        } else {
          elevenStOverseasFlatCache = this.allCategories
        }
    ...
    confirmSelection() {
      if (this.selectedCategory) {
        this.$emit('category-selected', {
          ...
          sellerType: this.activeTab // 'normal' 또는 'global'
        })
        this.closeModal()
      }
    },
