...
<script>
import naverCategories from '@/assets/naver_category.json'

// 네이버 카테고리 캐시 (첫 로드 이후 재사용)
let naverFlatCache = null

export default {
  name: 'CategoryMappingModal',
  props: {
    isVisible: {
      type: Boolean,
      default: false
    ...
    platform: {
      type: String,
      default: 'naver', // 'naver' or 'coopang'
  ...
  methods: {
    ...
    loadCategories(callback) {
      if (naverFlatCache) {
        this.allCategories = naverFlatCache
        if (callback) callback()
        return
      }
      this.categoriesLoading = true
      setTimeout(() => {
        this.allCategories = naverCategories // 이미 평면 구조
        naverFlatCache = this.allCategories
