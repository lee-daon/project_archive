 ...
 */
export async function processNewProducts(products) {
  ...
  for (const product of products) {
    const existing = await db.findByProductId(product.productId);
    if (existing) {
      duplicationCount++;
      ...
      await db.updateProductInfo(product.productId, updateData);
    } else {
      target.push(product);
    }
  }
